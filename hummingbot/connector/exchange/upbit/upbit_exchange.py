...
class UpbitExchange(ExchangePyBase):
    ...
    def __init__(self,
        ...
        self._last_trades_poll_upbit_timestamp = 1.0
    ...
    @staticmethod
    def to_hb_order_type(upbit_type: str) -> OrderType:
        return OrderType[upbit_type]
    ...
    async def _user_stream_event_listener(self):
        ...
        async for event_message in self._iter_user_event_queue():
            try:
                ...
                # Refer to https://github.com/upbit-exchange/upbit-official-api-docs/blob/master/user-data-stream.md
                # As per the order update section in Upbit the ID of the order being canceled is under the "C" key
    ...
    async def _update_order_fills_from_trades(self):
        ...
        in case Upbit's user stream events are not working.
        NOTE: It is not required to copy this functionality in other connectors.
        This is separated from _update_order_status which only updates the order status without producing filled
        events, since Upbit's get order endpoint does not return trade IDs.
        ...
        if (long_interval_current_tick > long_interval_last_tick
            ...
            query_time = int(self._last_trades_poll_upbit_timestamp * 1e3)
            self._last_trades_poll_upbit_timestamp = self._time_synchronizer.time()
